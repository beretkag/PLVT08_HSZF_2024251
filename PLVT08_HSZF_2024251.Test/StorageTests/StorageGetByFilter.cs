...
{
    [TestFixture]
    public class StorageGetAllTests
    {
        ...
        {
            var storages = storageService.GetByFilter(x => x.Id == true);
            storageProvider.Verify(x => x.GetAll(), Times.Once());
            Assert.That(() => storages.Count, Is.EqualTo(1));
            Assert.True(storages.First().Id);
        }
    }
}